<template>
  <div id="app">
    <div class="header">
      ...
      >
        <el-menu-item index="/">
          ...
          <span class="logo-text" v-if="!is_demo">pagium</span>
          <span class="logo-text" v-else>pagium 示例</span>
        </el-menu-item>
        <el-menu-item index="/create">编辑器</el-menu-item>
      </el-menu>
    </div>
    <div class="btn-group" :class="{['btn-group-show']:showBtn}">
      <el-dropdown @command="selectPage" size="medium" trigger="click" v-if="!is_demo">
        <span class="root-dropdown">
          根组件: {{$store.state.page || '无'}}
        ...
        <el-dropdown-menu slot="dropdown">
          ...
          <el-dropdown-item
            v-for="page in Object.keys($store.state.pages)"
            :key="page"
            :command="page"
      ...
      <el-popover placement="bottom" trigger="hover">
        <span class="save-path" v-if="!is_demo">输出路径：{{savePath}}</span>
        <span class="save-path" v-else>下载到本地</span>
        <el-button slot="reference" type="primary" size="small" @click="save" :loading="saving">生成</el-button>
      </el-popover>

      <el-button type="info" size="small" @click="preview" :loading="previewing" v-if="!is_demo">预览</el-button>

      <el-popover placement="bottom" trigger="hover" v-if="!is_demo">
      ...
      <el-button type="success" size="small" @click="openSaveAsTemplate" v-if="!is_demo">保存为模板</el-button>
    ...
    <el-dialog title="模板信息" :visible.sync="saveAsTemplateDialog.show" @close="closeSaveAsTemplate" v-if="!is_demo">
      ...
      <div slot="footer">
        ...
        <el-button
          ...
          :loading="saveAsTemplateDialog.commiting"
        >保 存</el-button>
      </div>
    </el-dialog>
  </div>
...
export default {
  data() {
    return {
      ...
      is_demo: !!process.env.PAGIUM_DEMO
    };
  },
  created() {
    //获取所有根组件
    !this.is_demo && this.$http
      .get("/pages")
      .then(({ data }) => {
    ...
    save() {
      ...
      if(this.is_demo){
        ...
        .then(({ data }) => {
          ...
          link.setAttribute("download", 'output.vue');
          ...
          link.dispatchEvent(event);
            this.saving = false;
          this.$message.success(`output.vue 生成成功`);
        })
        .catch(err => {
          this.saving = false;
